...
def predit_image(net, 
    ...
    #eval()是将神经网络模型切换到推断模式的方法，停止学习，禁用一些特定的训练相关操作。
    net.eval()
    with torch.no_grad():
        ...
        if pred is not None:
            ...
            if(save):
                ...
                for _, bbox in enumerate(pred):
                    #添加一条判断，如果CP/CE等于1，说明检测失败，该条数据作废
                    #####################################################
                    if(bbox[7]==1):
                        pass
                    #####################################################
...
def get_args():
    ...
    parser.add_argument('-m', '--model', metavar='M', type=str, default=r"lan4/TRD_final.pth",
    ...
    #                     help='TRD model path', dest='model')       
    parser.add_argument('-i', '--input', metavar='IN', type=str, default=r'DataSet/resizedImgSet1',
