...
class TRD(nn.Module):
    ...
    def forward(self, x):
        # 标记一下，这里很奇怪，明明x.shape跟self.inage_size是相等的，但是assert这行总报错出AsertionError，但是#掉就没有问题了。asert在这里干嘛呢？
        # print(x.shape[-1])
        # print(x.shape[-2])
    ...
    def bigdetect(self, image, transform, overlap, score_thresh = 0.7, iou_thresh = 0.5, cd_thresh = 0.1, device = None):
        ...
        # - 5 表示4个像素及以内的目标就不检测了
        # 我把 byc = (ih - overlap + step - 5)/step  bxc = (iw - overlap + step - 5)/step里的-5删了
        byc = (ih - overlap + step )/step
        bxc = (iw - overlap + step )/step
        byc = int(byc)
        bxc = int(bxc)
        uls = []
        for by in range(byc):
...
class TRDLoss(nn.Module):
    #num_classes原来是20.我改成了1; is_abs_bbox原来是False，我改成了True
    def __init__(self,bboxw_range,image_size = 416, num_classes=1, is_abs_bbox=True):
