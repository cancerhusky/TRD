...
from bbox_tr import bbox_tr_2_4pt_2
        ...
        if file_ext.lower() == '.txt':
            ...
            for line in gts_lbl:
                ...
                if (len(parts) > 6): 
                    ...
                    parts = [i.strip(',') for i in parts]
                    lbl = int(parts[0])
                    bbox = [float(x.strip()) for x in parts[1:7]]
                    bbox_4pt = bbox_tr_2_4pt_2(bbox)
...
if __name__ == '__main__':
    # aps = eval_net(r'lan4\lanl',r'lan4\SampleImages')
    # 以上基本是原文
    aps = eval_net(r'DataSet/imgHub',r'DataSet/resizedImgSet1')
    mAP = np.mean(aps)
    print(aps)
    print("mAP=%.7f"%mAP)
